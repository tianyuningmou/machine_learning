...
DATE CREATED:  @Time : 2018/4/19 下午4:13
...
CHANGE:  : 
MODIFIED: : @Time : 2018/4/19 下午4:13
"""
...
if __name__ == "__main__":
    ...
    print(a)
    # # nspace函数通过指定起始值、终止值和元素个数来创建数组，缺省包括终止值
    ...
    t = leastsq(residual, np.ndarray([0, 0, 0]), args=(x, y))
    theta = t[0]
    print('真实值：', A, B, C)
    print('预测值：', theta)
    ...
    t = leastsq(residual2, np.ndarray([3, 1]), args=(x, y))
    theta = t[0]
    print('真实值：', A, w)
    print('预测值：', theta)
    ...
    # 8.2 使用scipy计算函数极值
    a = opt.fmin(f, 1)
    b = opt.fmin_cg(f, np.ndarray([1]))
    c = opt.fmin_bfgs(f, np.ndarray([1]))
    print(a, 1/a, math.e)
    print(b)
    print(c)
